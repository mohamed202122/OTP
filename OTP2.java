    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < arr[10].length; i++) {
            ...
            for (int guess = 32; guess < 127; guess++) {
                ...
                if (flag) {
                    possible[i][h++] = (char) guess;
                }
                if (flag2) {
                    possible2[i][g++] = (char) guess;
                }
            }
            if (possible2[i][0] != '\0') {
                target += possible2[i][0];
        ...
        while (true) {
            try {
                ...
                System.out.println("select index character [0:91] ");
                ...
                System.out.println("There are suggested letters: " + target.charAt(index));
                if (possible2[index][0] != '\0') {
                    System.out.print("[ ");
                    for (int j = 0; j < possible2[index].length; j++) {
                        if (possible2[index][j] == '\0') {
                            break;
                        }
                        System.out.print(possible2[index][j] + ", ");
                    }
                    System.out.println("]\n");
                } else {
                    System.out.print("[ ");
                    for (int j = 0; j < possible[index].length; j++) {
                        if (possible[index][j] == '\0') {
                            break;
                        }
                        System.out.print(possible[index][j] + ", ");
                    }
                    System.out.println("]\n");
                }
                ...
                System.err.println("insert index between [0, 91] ");
            }
